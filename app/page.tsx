...
import { LampDesk, MessageCircleCode, Upload } from "lucide-react";
...
export default function Home() {
  ...
  const onSubmit = async () => {
    if (prompt === "") {
      toast.error("Não é possivel enviar uma pergunta vazia!");
      return;
    }
  ...
  return (
    <main className={`flex flex-col items-center h-screen gap-4`}>
      ...
      <div className="flex gap-2 items-center mb-5">
        <LampDesk size={64} />
        <span className="text-3xl font-bold">Zollak Chatbot + Gemini IA</span>
      </div>
      ...
      <footer className="mt-auto text-center text-xs text-gray-500">
        <span>
          <p className="">Em breve novas atualizações...</p>
        </span>
      </footer>
    </main>
  );
